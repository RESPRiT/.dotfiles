...
The consumer is the committed base statusline (`claude-global/statusline-command.sh`): it renders the time as `[HH:MM]` at the end of the line (white hour; light-orange brackets and minutes; dim `|` divider before it; seconds dropped), after any machine-local `extra` segments and the session-start stamp. Before the session's first Stop there's no file and the segment is simply absent.
...
The base statusline renders it as `[HH:MM TZ]` (white hour; cornflower — 256-color 69 — brackets, minutes, and timezone; seconds dropped), placed between the machine-local `extra` segments and the last-message `[HH:MM]`. Unlike the last-message stamp it never dims — the start time is immutable, so it's never stale.
