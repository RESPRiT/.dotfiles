...
A `Stop` hook (`claude-global/hooks/stop-last-message-time.sh`) records when Claude's last visible message landed, for display in the statusline. On every Stop event it writes `<epoch> <HH:MM:SS> <TZ>` to `~/.claude/last-stop/<session_id>` (and prunes entries older than 7 days). Stop hooks can block a stop and force continued work, so any given write may not be "the last message" — but the next Stop simply overwrites the file, so it always converges to the timestamp of the final reply the user actually sees; no `stop_hook_active` special-casing is needed.

The consumer is the committed base statusline (`claude-global/statusline-command.sh`): it renders the time as `[HH:MM TZ]` at the end of the line (white hour; light-orange brackets, minutes, and timezone; dim `|` divider before it; seconds dropped), after any machine-local `extra` segments, replacing the session-start stamp (see below) once it exists. Before the session's first Stop there's no file and the segment is simply absent; a legacy two-field file (pre-TZ) renders without the zone until the next Stop overwrites it.

**In-flight dimming.** A `UserPromptSubmit` companion (`claude-global/hooks/user-prompt-last-message-stale.sh`) stamps the prompt time to `<session_id>.prompt` in the same directory. The statusline renders the whole `[HH:MM TZ]` in the session-id's dim style while prompt-epoch ≥ stop-epoch (a turn is in flight — the displayed time no longer describes the latest visible message), and the next Stop's fresher epoch lights it back up. The epoch comparison self-resolves, so there's no marker-deletion handshake between the two hooks; the Stop hook's 7-day prune covers `.prompt` files too.

The `.prompt` stamp lands at Enter, but Claude Code only re-runs the statusline command after assistant messages (debounced 300ms) and a few UI events — prompt submission is not a refresh trigger, so the dim used to appear only once the response started. `statusLine.refreshInterval: 1` in the settings base adds a 1s polling re-run on top of the event-driven updates, making the dim appear within ~1s of Enter.

**Stale reddening.** When the displayed boundary stamp is 8+ hours old (`now − epoch ≥ STALE_SECS`, compared against the stamp file's epoch field on every render), the `[HH:MM TZ]` swaps its accent color for the soft red used for dirty branches (white hour kept, same schema as the orange/cornflower states) — the session has sat idle long enough that the time deserves a flag rather than glanceable styling. Applies to both the last-message and session-start stamps; in-flight dimming wins when both conditions hold (a just-submitted prompt means the stamp is about to refresh anyway). The 1s polling refresh means the flip to red happens live, not only on the next event. A malformed epoch field fails the numeric guard and falls back to normal styling. When the stale-red stamp is from a previous *local calendar day*, a red ` (N days ago)` suffix follows the bracket — `[08:14 PDT] (2 days ago)` — singular for one day. Day boundaries are computed from the current UTC offset (`date +%z`, portable across BSD/GNU date), so a DST shift since the stamp can mis-bucket within an hour of midnight; acceptable for a glanceable hint. Same-day red stamps (8–24h, possible for early-morning stamps viewed that evening) carry no suffix.
...
The base statusline renders it as `[HH:MM TZ]` (white hour; cornflower — 256-color 69 — brackets, minutes, and timezone; seconds dropped), after any machine-local `extra` segments. It completes the fencepost pattern: the last-message stamp marks the boundary before every turn except the first, and the start stamp is fencepost zero, so every turn has a visible "previous boundary" time. Only the most recent boundary stamp is shown — the start stamp stands in until the first Stop produces a last-message stamp, then yields to it for the rest of the session (the cornflower color disappears with it, so the segment's color also signals which boundary you're looking at).

The fencepost role extends to in-flight dimming: the boundary stamp dims while the following turn is in flight. For turn one that's the start stamp, so it renders dim while a `.prompt` stamp exists with no `last-stop/<session_id>` file yet (first prompt submitted, no Stop has landed); on later turns the last-message stamp handles it via the epoch comparison above.
