...
into the destination `~/.claude/settings.json`. Merge rules: objects deep-merge (right wins on scalars), string arrays concat-and-dedupe (so `permissions.allow` accumulates from both files), object arrays concat (so hook lists chain). The merge runs in `install.sh`, `hooks/post-merge`, and the in-session `claude-global/hooks/remerge-on-settings-edit.sh` PostToolUse hook — so a base or overlay edit propagates to dest without restarting Claude. Direct agent edits to `~/.claude/settings.json` are blocked by the `protect-settings.sh` PreToolUse hook — agents are directed to the base or overlay. Writes that bypass that hook (Claude Code's own `/config`, `/permissions`, settings UI) land in dest and get clobbered on the next merge; before clobbering, `merge-settings.sh` appends a unified diff to `.state/claude-settings-drift.log` (rotated past 1MB) and the in-session hook surfaces the event via `additionalContext`, so intentional edits can be promoted to base or overlay rather than silently lost. The destination is a real file, not a symlink, because Claude Code has a known bug where symlinked `settings.json` triggers permission failures (anthropics/claude-code#3575).
