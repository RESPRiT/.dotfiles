...
The consumer is the committed base statusline (`claude-global/statusline-command.sh`): it renders the time as `[HH:MM TZ]` at the end of the line (white hour; light-orange brackets, minutes, and timezone; dim `|` divider before it; seconds dropped), after any machine-local `extra` segments and the session-start stamp. Before the session's first Stop there's no file and the segment is simply absent; a legacy two-field file (pre-TZ) renders without the zone until the next Stop overwrites it.
