...
**Stale reddening.** When the displayed boundary stamp is 8+ hours old (`now − epoch ≥ STALE_SECS`, compared against the stamp file's epoch field on every render), the whole `[HH:MM TZ]` renders in the soft red used for dirty branches instead of its normal colors — the session has sat idle long enough that the time deserves a flag rather than glanceable styling. Applies to both the last-message and session-start stamps; in-flight dimming wins when both conditions hold (a just-submitted prompt means the stamp is about to refresh anyway). The 1s polling refresh means the flip to red happens live, not only on the next event. A malformed epoch field fails the numeric guard and falls back to normal styling.
