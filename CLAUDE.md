...
- **`install.sh`** (POSIX — Linux, macOS, WSL, Git Bash): symlinks the bash/zsh/vim/tmux/ghostty config into `$HOME`, installs CLI tools (rust, go, zoxide, atuin, keychain, tmux 3.5+), wires the post-merge git hook, optionally enables sshd with key-only auth + the committed `ssh/authorized_keys` (SSH over Tailscale; macOS/Linux only — skipped on WSL/Git Bash, where the Windows installer owns it), and runs pending migrations. The sshd hardening drop-in is named `010-dotfiles.conf` so it sorts ahead of macOS's `100-macos.conf` (sshd is first-match-wins per keyword).
