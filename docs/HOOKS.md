...
The consumer is the committed base statusline (`claude-global/statusline-command.sh`): it renders the time as `[HH:MM]` at the end of the line (white hour; light-orange brackets and minutes; dim `|` divider before it; seconds dropped), after any machine-local `extra` segments. On this machine, `~/.claude/statusline-command.local.sh` additionally uses the file's existence to displace the metacog trace type label (`NEW`/`DO`/`PLAN`/…) — the id keeps its type-derived color, so the session type is still hinted. Before the session's first Stop there's no file and the segment is simply absent.
